...
function handle(customObj) {
  ...
  if (orderCreateDateDelay < currentDate) {
    switch (customObj.custom.eventCode) {
      case 'AUTHORISATION':
        var paymentInstruments = order.getPaymentInstruments(); // Move adyen log request to order paymet transaction
    ...
    if (!refusedHpp) {
      ...
      */
      ...
      order.custom.Adyen_eventCode = customObj.custom.eventCode; // Payment Method must be persistent. When payment is cancelled, Adyen sends an empty string here
