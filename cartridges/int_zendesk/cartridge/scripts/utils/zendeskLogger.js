...
/**
 * @module scripts/utils/zendeskLogger
 *
...
function logMessage(message, severityLevel, logLocation) {
    ...
    var loggerClass = 'int_zendesk';

    switch (severityLevel) {
        case 'debug':
            var zendeskDebugLogEnabled = Site.getCurrent().getPreferences().custom.zendeskDebugLogEnabled;
            if (zendeskDebugLogEnabled) {
                Logger.getLogger(loggerClass).debug(logLocation + ' : ' + message);
            }
            break;

        case 'info':
            var zendeskInfoLogEnabled = Site.getCurrent().getPreferences().custom.zendeskInfoLogEnabled;
            if (zendeskInfoLogEnabled) {
