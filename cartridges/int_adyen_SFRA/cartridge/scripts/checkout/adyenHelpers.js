  ...
  if (order.totalNetPrice !== 0.0) {
    ...
    if (!result.error) {
      for (var i = 0; i < paymentInstruments.length; i++) {
        ...
        var authorizationResult = void 0;
...
function validatePayment(req, currentBasket) {
  ...
  var result = {};
  var applicablePaymentMethods = PaymentMgr.getApplicablePaymentMethods(currentCustomer, countryCode, paymentAmount);
  var applicablePaymentCards = creditCardPaymentMethod.getApplicablePaymentCards(currentCustomer, countryCode, paymentAmount);
  var invalid = true;
  ...
  for (var i = 0; i < paymentInstruments.length; i++) {
    ...
    if (paymentMethod && applicablePaymentMethods.contains(paymentMethod)) {
      if (PaymentInstrument.METHOD_CREDIT_CARD.equals(paymentInstrument.paymentMethod)) {
        var card = PaymentMgr.getPaymentCard(paymentInstrument.creditCardType); // Checks whether payment card is still applicable or if there is a credit card token set.
